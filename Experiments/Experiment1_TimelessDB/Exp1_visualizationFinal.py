...
def vectors_calculation2(results, rows):
    ...
    for i in range(rows + 1):
        ...
        if i == 0:
            ...
            LDA_PostProb_e1.append(results['1_ErrorLDA_PostProb'].loc[results['# shots'] == i].mean())
            LDA_Labels_e1.append(results['1_ErrorLDA_Labels'].loc[results['# shots'] == i].mean())
            LDA_PostProb_MSDA_e1.append(results['1_ErrorLDA_PostProb_MSDA'].loc[results['# shots'] == i].mean())

            QDA_PostProb_e1.append(results['1_ErrorQDA_PostProb'].loc[results['# shots'] == i].mean())
            QDA_Labels_e1.append(results['1_ErrorQDA_Labels'].loc[results['# shots'] == i].mean())
            QDA_PostProb_MSDA_e1.append(results['1_ErrorQDA_PostProb_MSDA'].loc[results['# shots'] == i].mean())

            LDA_PostProb_e2.append(results['2_ErrorLDA_PostProb'].loc[results['# shots'] == i].mean())
            LDA_Labels_e2.append(results['2_ErrorLDA_Labels'].loc[results['# shots'] == i].mean())
            LDA_PostProb_MSDA_e2.append(results['2_ErrorLDA_PostProb_MSDA'].loc[results['# shots'] == i].mean())

            QDA_PostProb_e2.append(results['2_ErrorQDA_PostProb'].loc[results['# shots'] == i].mean())
            QDA_Labels_e2.append(results['2_ErrorQDA_Labels'].loc[results['# shots'] == i].mean())
            QDA_PostProb_MSDA_e2.append(results['2_ErrorQDA_PostProb_MSDA'].loc[results['# shots'] == i].mean())
...
analysis1(folder='resultsFinal2/', database='Cote', samplesInMemory=0, featureSet=1, times=4)
analysis1(folder='resultsFinal2/', database='Nina5', samplesInMemory=0, featureSet=1, times=4)
analysis1(folder='resultsFinal2/', database='EPN', samplesInMemory=0, featureSet=1, times=4)
