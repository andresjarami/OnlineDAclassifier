...
def model_incre_proposed3(currentModel, classes, trainFeatures, trainLabel, labeledGesturesFeatures,
                          labeledGesturesLabels, type_DA, CM, weakModel):
    ...
    # if type_DA == 'QDA':
    #     print('QDA total3', weightsUnlabeledGesture, trainLabel)
...
def model_incre_proposed2(currentModel, classes, trainFeatures, trainLabel, labeledGesturesFeatures,
                          labeledGesturesLabels, type_DA, CM, weakModel):
    predictedLabels = predicted_labels(trainFeatures, currentModel, classes, type_DA)
    CM[trainLabel - 1, :] += predictedLabels
    postProb_trainFeatures = predictedLabels / predictedLabels.sum()
    ...
    weightsUnlabeledGesture = ((postProb_trainFeatures2 * entrophyVector(postProb_trainFeatures2)) + (
            postProb_trainFeatures * entrophyVector(postProb_trainFeatures))) / 2

    # if type_DA == 'QDA':
    #     print('QDA total2', weightsUnlabeledGesture, trainLabel)
...
def model_incre_weight_Nigam(currentModel, classes, trainFeatures, trainLabel, type_DA, CM, weakModel, weight_Nigam):
    ...
    gesture_N = np.size(trainFeatures, axis=0)
    postProb_trainFeatures2 = predicted_labels(trainFeatures, weakModel, classes, type_DA) / predictedLabels.sum()
    weightsUnlabeledGesture = postProb_trainFeatures2 * weight_Nigam
...
def model_incre_label_threshold(currentModel, classes, trainFeatures, trainLabel, type_DA, CM, weakModel, threshold):
    predictedLabels = predicted_labels(trainFeatures, currentModel, classes, type_DA)
    CM[trainLabel - 1, :] += predictedLabels

    t = time.time()
    postProb_trainFeatures2 = predicted_labels(trainFeatures, weakModel, classes, type_DA) / predictedLabels.sum()
    if np.max(postProb_trainFeatures2) > threshold:
        gesture_mean = np.mean(trainFeatures, axis=0)
        gesture_cov = np.cov(trainFeatures, rowvar=False)
        gesture_N = np.size(trainFeatures, axis=0)
...
def model_incre_labels(currentModel, classes, trainFeatures, trainLabel, type_DA, CM, weakModel):
    predictedLabels = predicted_labels(trainFeatures, currentModel, classes, type_DA)
    CM[trainLabel - 1, :] += predictedLabels
    t = time.time()
    gesture_mean = np.mean(trainFeatures, axis=0)
...
def model_incre_sequential_labels_stateArt(currentModel, classes, trainFeatures, trainLabel, type_DA, CM, weakModel):
    ...
    for sample in range(len(trainFeatures[:, 0])):
        ...
        if type_DA == 'LDA':
            ...
            cla2 = DA_Classifiers.predictedModelLDA(x, weakModel, classes, weakModel.loc[0, 'LDAcov'])
        elif type_DA == 'QDA':
            cla = DA_Classifiers.predictedModelQDA(x, currentModel, classes)
            cla2 = DA_Classifiers.predictedModelQDA(x, weakModel, classes)

        CM[trainLabel - 1, cla - 1] += 1

        weightsUnlabeledGesture = np.zeros(classes)
        weightsUnlabeledGesture[cla2 - 1] = 1
        gesture_cov = 0
        gesture_N = 1
        t = time.time()
